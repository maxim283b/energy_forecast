...
LOG_OFFSET = 50  # must match train_optuna.py


def add_model_predictions(df: pd.DataFrame) -> pd.DataFrame:
    """Add real model predictions for Evidently regression reports."""
    ...
    feature_cols = [c for c in df.columns if c not in {"timestamp", "target", "prediction"}]
...
def evaluate_current_model(df: pd.DataFrame, split_ratio: float = 0.8) -> dict[str, float]:
    if not MODEL_PATH.exists():
        return {}
...
def evaluate_retrain_trigger(
    ...
    if max_psi > PSI_THRESHOLD:
        reasons.append(
            f"PSI {max_psi:.3f} on '{max_psi_feature}' > {PSI_THRESHOLD}"
        )
    ...
    if current_mae is not None and baseline.get("mae"):
        ...
        if mae_increase > MAE_INCREASE_RATIO:
            reasons.append(
                f"MAE increased {mae_increase:.1%} vs baseline ({current_mae:.2f} vs {baseline['mae']:.2f})"
            )
    ...
    if current_r2 is not None and baseline.get("r2") is not None:
        ...
        if r2_drop > R2_DROP_THRESHOLD:
            reasons.append(
                f"R2 dropped {r2_drop:.3f} vs baseline ({current_r2:.3f} vs {baseline['r2']:.3f})"
            )
