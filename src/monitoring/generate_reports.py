...
try:
    ...
    raise ImportError(
        "Evidently is not installed. Install dependencies and rerun."
    ) from exc
...
def _split_reference_current(df: pd.DataFrame, ratio: float = 0.8) -> tuple[pd.DataFrame, pd.DataFrame]:
    split_idx = int(len(df) * ratio)
    return df.iloc[:split_idx].copy(), df.iloc[split_idx:].copy()


def _save_report(report: Report, reference: pd.DataFrame, current: pd.DataFrame, name: str) -> None:
...
def _log_reports_to_mlflow() -> None:
    mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
    mlflow.set_experiment(MLFLOW_EXPERIMENT)

    with mlflow.start_run(run_name=f"drift_monitoring_{datetime.now(timezone.utc):%Y%m%d_%H%M%S}"):
        ...
        if TRIGGER_STATUS_PATH.exists():
            mlflow.log_artifact(str(TRIGGER_STATUS_PATH), artifact_path="drift_reports")
...
def main() -> None:
    ...
    _save_report(Report(metrics=[DataDriftPreset()]), reference_data, current_data, "data_drift")
    _save_report(Report(metrics=[TargetDriftPreset()]), reference_data, current_data, "target_drift")
    ...
    if current_metrics and (
        os.getenv("SET_BASELINE", "false").lower() == "true"
        or not BASELINE_METRICS_PATH.exists()
    ):
        save_baseline_metrics(current_metrics["mae"], current_metrics["r2"])

    trigger_status = evaluate_retrain_trigger(reference_data, current_data, current_metrics)
    trigger_status["checked_at"] = datetime.now(timezone.utc).isoformat()

    TRIGGER_STATUS_PATH.write_text(json.dumps(trigger_status, indent=2), encoding="utf-8")
    print(json.dumps(trigger_status, indent=2))

    _log_reports_to_mlflow()
    print(f"Reports saved to {REPORTS_DIR} and logged to MLflow ({MLFLOW_TRACKING_URI})")

    if PREDICTIONS_PATH.exists():
        print(f"Latest predictions: {PREDICTIONS_PATH}")

    if trigger_status["should_retrain"] and os.getenv("AUTO_RETRAIN", "false").lower() == "true":
        try:
