...
def load_and_prepare_data(file_path):
    """Загрузить и подготовить данные с лагами"""
...
def create_features(df):
    ...
    feature_columns = time_columns + lag_columns
...
def plot_predictions(y_test, y_pred, save_path):
    ...
    plt.title('Energy Price Prediction with Lags')
    plt.legend()
...
def main():
    """Основная функция обучения с лагами"""
    ...
    print(f"  Time features: {feature_columns[:5]}")
    print(f"  Lag features: {feature_columns[5:]}")
    ...
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        mlflow.sklearn.log_model(model, "random_forest_with_lags")
        mlflow.log_artifact(plot_path)

        print(f"\nExperiment saved to MLflow")
        print(f"Run ID: {mlflow.active_run().info.run_id}")

    print("\nTraining with lags completed")
