...
def render_report(report_name: str, title: str) -> None:
    ...
    if report_path.exists():
        ...
        st.info(f"Report not found: {report_path}. Run: python src/monitoring/generate_reports.py")


def render_predictions() -> None:
    st.subheader("Latest Forecast")
    if not PREDICTIONS_PATH.exists():
        st.warning("No local predictions yet. Run training and inference first.")
        return
...
def call_retrain_api() -> None:
    ...
    try:
        ...
        st.info(
            "Запусти API во втором терминале:\n\n"
            "`uvicorn src.api.retrain_server:app --host 127.0.0.1 --port 8001`"
        )
...
def main() -> None:
    ...
    st.caption("Local dashboard: inference, drift monitoring, and retraining")
